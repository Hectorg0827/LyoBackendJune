...
CMD ["bash", "-c", "mkdir -p /tmp/prometheus_multiproc_dir && gunicorn lyo_app.enhanced_main:app --worker-class uvicorn.workers.UvicornWorker --bind 0.0.0.0:${PORT} --workers ${WORKERS:-4} --timeout 120"]
