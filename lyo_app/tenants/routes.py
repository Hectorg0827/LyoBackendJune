...
# NOTE: The per-user API-key management endpoints (POST/GET /api-keys and
# DELETE /api-keys/{id}) were removed. They depended on User.organization_id,
# which is not a column on the User model, so every call 500'd. Organization
# API keys are provisioned out-of-band (see setup_multi_tenant.py /
# lyo_app.auth.api_key_auth.create_api_key) rather than self-served here.
